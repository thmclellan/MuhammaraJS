...
exports._readInfo = function _readInfo() {
  if (!this.isNewPDF && !this.infoDictionary) {
    // Use the existing password-aware reader; a fresh copying context
    // has no access to the decryption password and fails on
    // encrypted sources.
    const pdfReader = this.pdfReader;
    const trailer = pdfReader.getTrailer();
    const hasInfo = trailer && trailer.toJSObject().Info;
    const infoDict = hasInfo
      ? pdfReader.queryDictionaryObject(trailer, "Info")
      : null;
